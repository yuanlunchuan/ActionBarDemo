...
//Rare exceptions to indicate a bug in the _androidiosactionbar code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_androidiosactionbar = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_androidiosactionbar]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.androidiosactionbar.module"
