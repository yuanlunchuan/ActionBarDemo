...
NSString * androidiosActionBar$ModuleRequireFormat = @"(function(exports){"
...
@implementation androidiosActionBarObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_androidiosactionbar);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create androidiosActionBar global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "androidiosActionBar" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_androidiosactionbar = [[androidiosActionBarObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_androidiosactionbar];
	
	NSString *_androidiosactionbarNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _androidiosactionbarNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_androidiosactionbar valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:androidiosActionBar$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.androidiosactionbar.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.androidiosactionbar.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
